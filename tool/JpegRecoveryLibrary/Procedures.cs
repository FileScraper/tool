...
namespace JpegRecoveryLibrary
{
    public class Procedures
    {
        ...
        public Tuple<String, String> procedure_3(String packetPath)
        {
            ...
            String packetfragPath = packetPath+"pkt_image_fragments";
            ...
            for (int i = 0; i < imgFragMemoryStreams.Count; i++)
            {
                ...
                String imageFragPath = packetfragPath + Path.DirectorySeparatorChar + "frag_" + i;
        ...
        public String runCMD(String proc, String command)
        {
        ...
        /*
         *Procedure 5 - Video Recovery
         * 
         */
        public Tuple<String, String> procedure_5(String rawPath, String isFile, String pythonFile, String pyFile)
        {
            String outFile = "" + rawPath;
            String directory = rawPath +"frames" + Path.DirectorySeparatorChar;

            Console.WriteLine("Running Procedure 5");
            if (!File.Exists(rawPath))
            {
                String msg = "Error - File does not exists.";
                return Tuple.Create(outFile, msg);
            ...
            //python orphanFrameRecovery.py rawPath
            //Check if file is jpeg or not
            FileStream tempFileStream = new FileStream(rawPath, FileMode.Open);

            String proc = pythonFile;
            String command = string.Format("{0} \"{1}\" {2}", pyFile,rawPath,isFile);

            string result = runCMD(proc, command);
            Console.WriteLine(result);
            List<String> listStrLineElements = result.Split(separator: '\n').ToList();
            listStrLineElements[0] = listStrLineElements[0].Remove(16, 17).Insert(16, "\n");
            return Tuple.Create(listStrLineElements[1], listStrLineElements[0]);
